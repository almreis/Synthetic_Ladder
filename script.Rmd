...
ratio.s1 <- ratio.s1[order(Name,S1.Expected_Abundance),]
ratio.s1 <- data.table(V1=ratio.s1[setdiff(1:56,seq(1,56,4)),Median]/ratio.s1[setdiff(1:56,seq(4,56,4)),Median])
p1 <- ggplot(ratio.s1,aes(V1))+
...
ratio.s2 <- ratio.s2[order(Name,S2.Expected_Abundance),]
ratio.s2 <- data.table(V1=ratio.s2[setdiff(1:56,seq(1,56,4)),Median]/ratio.s2[setdiff(1:56,seq(4,56,4)),Median])
p2 <- ggplot(ratio.s2,aes(V1))+
        geom_histogram(color="black",fill="gray")+
        scale_x_continuous(limits=c(0,4),breaks=0:4)+  
        labs(x="Ratio",y="Density")
...
meta_all_cn[P.value < 0.05,Significance:=TRUE]
...
for (norm_name in c('uqua','deseq2','tmm','normalize.quantiles')) {
  ...
  roc_plot <- ggplot(tmp,aes(d=Status,m=P.value,color=Norm))+
    geom_roc(n.cuts = 0)+
    coord_fixed()+
  ...
  plots[[i]] <- ggplot(tmp,aes(d=Status,m=P.value,color=Label))+
    geom_roc(n.cuts = 0)+
    coord_fixed()+
    geom_abline(slope=1,color="gray",linetype="dashed")+
    theme(legend.position = "bottom")+
